...
## 9A. Supabase Client Persistence Flow

Use this pattern when a client-only app must preserve user progress across incognito windows, browser restarts, sign-out/sign-in, or different devices.

### Table shape: key/value rows per user
For small app state, use one RLS-protected key/value table instead of many premature normalized tables:
...
1. Use Supabase only after `session.user.id` exists.
2. Keep `localStorage` only as a development fallback when Supabase env vars are missing.
3. Store values in `jsonb` as real JSON objects, not JSON strings.
4. Read defensively because old rows may already be stringified JSON:
...
Use `toStoredJson(value)` before `upsert()` into a `jsonb` column.

### Avoid the hydration overwrite bug

Do **not** autosave immediately after login, route load, or storage hydration. That can overwrite real cloud data with default empty state.
...
useEffect(() => {
  // Load cloud state first.
  dayDirty.current = false;
  setDay(loadedDayFromSupabase);
  setDayLoaded(true);
}, [storageScope]);
...
useEffect(() => {
  if (!loaded || !dayLoaded) return;
  if (!dayDirty.current) return;       // critical guard

  storage.set(dayKey, JSON.stringify(day))
    .then(() => { dayDirty.current = false; })
    .catch((error) => setSaveError(error.message));
}, [day, loaded, dayLoaded, storage]);
```

Also reset in-memory state when auth/storage scope changes:
...
### Sign-out must flush first

Before `supabase.auth.signOut()`, explicitly save the latest state and block sign-out if the write fails:
...
const signOut = async () => {
  try {
    await Promise.all([
      storage.set(dayKey, JSON.stringify(day)),
      storage.set(weekKey, JSON.stringify(week)),
    ]);
...
Never swallow cloud-write errors during auth/persistence debugging. Show a visible banner:

```tsx
{saveError && <div>Cloud save problem: {saveError}</div>}
```

This catches RLS problems, missing authenticated session, malformed env vars, and table/schema mismatch.

### Verification query

After saving a test entry in the app, verify rows exist:
...
Then sign out, sign back in with the same email, and verify the header/user indicator matches the same account.
...
| User saves progress, sees it in History, then it disappears after sign-out/sign-in | Autosave ran during auth/storage hydration and overwrote Supabase with default empty state | Add dirty refs; only autosave after user edits; reset state on `storageScope` changes; flush before sign-out |
| Supabase `jsonb` row exists but app history is empty after reload | App assumes `value` is always a JSON string, but Supabase returns `jsonb` as an object | Add `parseStoredValue()` that accepts both strings and objects; store new rows as real JSON |
| App appears to save but Supabase table has no rows | Cloud write error is swallowed, RLS rejects writes, or the app is using local fallback | Display `Cloud save problem: {message}`; confirm signed-in user id exists; verify Vercel has `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` |
| Header shows a different account after login | User signed in with a different email or confirmed a different Supabase user | Show signed-in email in the UI and verify `progress_entries.user_id` rows belong to that same account |
